...
static char s_min_to_breathe_text[3] = "1", s_instruct_text[27], s_min_text[25], s_lower_text[25], s_upper_text[27], s_start_time[11], s_end_time[11];
static time_t t;
...
static void upper_text_layer_update_proc(Layer *s_inside_text_layer, GContext *ctx) {
  graphics_draw_upper_text(ctx, bounds, s_animating, settings_get_displayText(), settings_get_textColor(), s_upper_text);
}

// Draws text at bottom of screen
static void lower_text_layer_update_proc(Layer *s_inside_text_layer, GContext *ctx) {
  graphics_draw_lower_text(ctx, bounds, s_animating, settings_get_bottomTextType(), settings_get_textColor(), s_lower_text);
}
...
static void animation_end_callback(void *data) {
  ...
  snprintf(s_upper_text, sizeof(s_upper_text), "%s", localize_get_upper_text());
  snprintf(s_lower_text, sizeof(s_lower_text), localize_get_min_breathed_today_text(), s_min_breathed_today);
...
static void main_animation_end(void *data) {
  ...
  if (complete == 0) {
    ...
    // Localized strings
    snprintf(s_lower_text, sizeof(s_lower_text), "%s", localize_get_well_done_text());
        
    // Hide heart rate text, and show the "Well done text" and then hides it after 2 seconds
    layer_set_hidden(s_lower_text_layer, false);
...
static void heart_rate_update_callback(void *context) {
  if (s_animating) {
    ...
    snprintf(s_upper_text, sizeof(s_upper_text), "%s", data_get_current_heart_rate_buffer()); // Show heart rate upon redraw of s_upper_text_layer
    if (!(strcmp(s_upper_text, "0 BPM") == 0))
      layer_set_hidden(s_upper_text_layer, false);
...
static void first_breath_out_callback(void *context) {
  ...
  snprintf(s_lower_text, sizeof(s_lower_text), "%s", localize_get_exhale_text());
  layer_set_hidden(s_upper_text_layer, true);
  layer_set_hidden(s_lower_text_layer, false);
...
static void first_breath_in_callback(void *context) {
  ...
  snprintf(s_upper_text, sizeof(s_upper_text), "%s", localize_get_inhale_text());
  layer_set_hidden(s_upper_text_layer, false);
#ifdef PBL_ROUND
    layer_set_hidden(s_inside_text_layer, true);
#endif
...
static void animation_start_callback(void *context) {
  ...
  snprintf(s_upper_text, sizeof(s_upper_text), "%s", localize_get_top_text(random_number));
  snprintf(s_lower_text, sizeof(s_lower_text), "%s", localize_get_bottom_text(random_number_2));
  layer_set_hidden(s_upper_text_layer, false);
  layer_set_hidden(s_lower_text_layer, false);
#else
