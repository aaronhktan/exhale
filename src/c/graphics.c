...
void graphics_draw_upper_text(GContext *ctx, GRect bounds, bool is_animating, int display_text, GColor textColor, char *upper_text) {
  ...
  GSize upper_text_bounds = graphics_text_layout_get_content_size("THAT IS A LOT OF STEPS TO TAKE IN JUST ONE DAY, INNIT?", fonts_get_system_font(FONT_KEY),
  ...
  if (is_animating) { // If animating, shows the normal text
    graphics_draw_text(ctx, upper_text, fonts_get_system_font(FONT_KEY), 
                       GRect((bounds.size.w - upper_text_bounds.w) / 2, PBL_IF_RECT_ELSE(5, 15), upper_text_bounds.w, upper_text_bounds.h),
    ...
    if (display_text == 3 && data_get_current_heart_rate() > 0) { // If heart rate monitor is enabled in configuration and is available, show heart rate
      ...
      switch(display_text) {
        ...
        case 2:
        case 3:
          graphics_draw_text(ctx, PBL_IF_HEALTH_ELSE(steps_buffer, upper_text), fonts_get_system_font(FONT_KEY), 
                       GRect((bounds.size.w - upper_text_bounds.w) / 2, PBL_IF_RECT_ELSE(5, 15), upper_text_bounds.w, upper_text_bounds.h),
                       GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);
          break;
      }
